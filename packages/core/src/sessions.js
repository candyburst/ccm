...
function loadLog() {
  ...
  try { return JSON.parse(readFileSync(SESSION_LOG, 'utf8')) }
  catch { return [] }
}
...
export function endSession(id, { exitCode, exitReason = EXIT_REASONS.NORMAL, switched = false, switchedTo = null, tokens = null } = {}) {
  const log   = loadLog()
...
export function getSessions({ limit = 50, account = null, from = null, to = null, exitReason = null } = {}) {
...
export function getSessionStats() {
  ...
  for (const e of log) {
    ...
    if (e.switched) { byAccount[e.account].switches++; switches++ }
...
export function cleanupOrphanedSessions(maxAgeMinutes = 60) {
  ...
  for (const e of log) {
    if (e.exitReason === EXIT_REASONS.RUNNING && new Date(e.startedAt).getTime() < cutoff) {
      ...
      e.durationSec = e.durationSec || Math.round((Date.now() - new Date(e.startedAt).getTime()) / 1000)
      changed = true
    }
  }
