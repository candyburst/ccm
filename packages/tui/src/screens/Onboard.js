...
export default function Onboard({ onComplete }) {
    ...
    return (
      <Box flexDirection="column" gap={1} paddingY={1}>
        ...
        <Box marginTop={1} flexDirection="column" gap={0}>
          ...
          <Text dimColor>
            You'll add an account, and optionally bind the current directory as a project.
          </Text>
        </Box>
        <Box marginTop={1}>
  ...
  if (step === 'project') {
    ...
    return (
      <Box flexDirection="column" gap={1} paddingY={1}>
        ...
        <Text>
          Binding means <Text bold>ccm run</Text> always uses account{' '}
          <Text color="cyan">{form.name}</Text> here.
        </Text>
        <SelectInput
          items={projectChoiceItems}
          onSelect={item => handleProjectChoice(item.value)}
        />
      </Box>
    )
  }
    ...
    return (
      <Box flexDirection="column" gap={1} paddingY={1}>
        <Text color="green" bold>
          ✓ Account "{form.name}" added and set as active.
        </Text>
        ...
        {initDir && (
          <Text dimColor>
            ✓ {process.cwd()} bound to "{form.name}"
          </Text>
        )}
        ...
        <Box marginTop={1} flexDirection="column" gap={0}>
          ...
          <Text>
            {' '}
            2. Open the TUI at any time with{' '}
            <Text bold color="cyan">
              ccm
            </Text>
          </Text>
  ...
  const field =
    step === 'credential' && form.type === AUTH.API_KEY
      ? 'key'
      : step === 'credential'
        ? 'email'
        : step
