  ...
  if (mode === 'gitignore' && gitignoreProject) {
    ...
    return (
      <Box flexDirection="column" gap={1}>
        ...
        <Text dimColor>
          .ccm-project.json is not listed in .gitignore for "{gitignoreProject.name}". Adding it
          keeps the file out of your git history.
        </Text>
        <SelectInput
          items={items}
          onSelect={item => {
    ...
    <Box flexDirection="column" gap={1}>
      ...
      {projects.length === 0 && (
        <Box paddingY={1} flexDirection="column" gap={0}>
          ...
          <Text dimColor>
            Run <Text color="cyan">ccm project init</Text> inside a project directory.
          </Text>
      ...
      {projects.map((p, i) => {
        ...
        return (
          ...
          >
            <Box gap={2}>
              <Text bold={isSel} color={isSel ? 'white' : 'gray'}>
                {p.name}
              </Text>
