...
function loadConfig() {
  ...
  catch { return { autoSwitch: true, keepSessionLog: true, maxLogEntries: 500 } }
}
...
const TOGGLES = [
  { key: 'autoSwitch',     label: 'Auto-switch on credit limit', desc: 'Rotates to next account when credits run out' },
  { key: 'keepSessionLog', label: 'Keep session log',            desc: 'Logs each run to ~/.ccm/session-log.json' },
]
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      <Box flexDirection="column" paddingX={1}>
        <Text bold dimColor>Storage</Text>
