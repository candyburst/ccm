...
function TermLine({ line }) {
  ...
  return (
    <div style={{ color: style.color, fontFamily: 'inherit', fontSize: 12, lineHeight: 1.5, whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
      {line.text}
    </div>
  )
}
...
export default function RunSession() {
  ...
  useEffect(() => {
    ...
    const off = window.ccm.runner.onEvent(({ sessionId: sid, type, data }) => {
      if (type === 'started') {
        addLine('info', `[ccm] Session started · account: ${data.accountName}${data.projectName ? ` · project: ${data.projectName}` : ''}`)
      } else if (type === 'stdout') {
        addLine('stdout', data)
      } else if (type === 'stderr') {
        addLine('stderr', data)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {isRunning
            ...
            : <button className="primary" onClick={start} disabled={!accountName}>▶ start</button>
          }
          <button className="ghost" onClick={clearLog} disabled={isRunning}>clear</button>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1 }}>
          ...
          <input
            ...
            disabled={isRunning}
            onKeyDown={e => { if (e.key === 'Enter' && !isRunning && accountName) start() }}
          />
        </div>
      ...
      {creditWarn && (
        <div style={{
          background: '#2a1e00', border: '1px solid #5a4010',
          borderRadius: 6, padding: '10px 14px',
          color: 'var(--yellow)', fontSize: 12,
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        }}>
          <span>⚠ Credit limit hit on "<strong>{creditWarn}</strong>" — switching to next account automatically</span>
          <button className="ghost" style={{ fontSize: 11 }} onClick={() => setCreditWarn(null)}>dismiss</button>
        </div>
      )}
      ...
      <div style={{
        ...
        {lines.length === 0 && (
          <div style={{ color: 'var(--text3)', fontSize: 12 }}>
            {isRunning ? 'Waiting for output…' : 'Select an account and press ▶ start to launch Claude Code.'}
          </div>
        )}
        {lines.map(l => <TermLine key={l.id} line={l} />)}
        {isRunning && (
          <span style={{ display: 'inline-block', width: 8, height: 13, background: 'var(--accent)', marginLeft: 1, animation: 'blink 1s step-end infinite' }} />
        )}
        <div ref={bottomRef} />
      </div>
