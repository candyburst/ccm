  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <span style={{ color: isActive ? 'var(--accent)' : isDisabled ? 'var(--red)' : 'var(--text3)', fontSize: 16 }}>
          {isActive ? '●' : isDisabled ? '✗' : '○'}
        </span>
        <div style={{ flex: 1 }}>
        ...
        <span className={`badge ${account.type === 'api_key' ? 'api-key' : 'email'}`}>{typeLabel}</span>
      ...
      <div style={{ display: 'flex', gap: 8, paddingLeft: 26 }}>
        {!isActive && !isDisabled && (
          <button className="primary" style={{ fontSize: 12 }} onClick={() => onSetActive(account.name)}>
            set active
          </button>
        )}
        ...
        <button className="ghost" style={{ fontSize: 12 }} onClick={() => onToggleDisable(account.name, !isDisabled)}>
...
export default function Dashboard({ onAdd }) {
  ...
  async function setActive(name) {
    try { await window.ccm.accounts.setActive(name)
  ...
  async function toggleDisable(name, disabled) {
    ...
    if (res && !res.ok) { setStatus(`✗ ${res.error}`); return }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      {status && (
        <div style={{ color: 'var(--accent)', background: 'var(--bg3)', border: '1px solid var(--accent2)', borderRadius: 6, padding: '8px 14px', fontSize: 12 }}>
          {status}
        </div>
      )}

      <div>
        <div style={{ color: 'var(--text2)', fontSize: 11, marginBottom: 12, textTransform: 'uppercase', letterSpacing: 1 }}>
          accounts ({accounts.length})
        </div>
        {accounts.length === 0 ? (
          <div style={{ color: 'var(--text3)', padding: '24px 0' }}>
            No accounts yet.{' '}
            <span style={{ color: 'var(--accent)', cursor: 'pointer' }} onClick={onAdd}>Add one →</span>
          </div>
        ) : (
