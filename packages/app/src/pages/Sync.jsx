    ...
    >
      ...
      <div
        style={{
          ...
          flexShrink: 0,
          marginLeft: 16,
          background: value ? 'var(--accent2)' : 'var(--border2)',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 640 }}>
        ...
        {gitStatus?.isGitRepo ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              {gitStatus.hasRemote ? (
                <StatusBadge ok label="remote connected" />
              ) : (
                <StatusBadge ok={false} label="no remote" />
              )}
              {gitStatus.isDirty ? (
                <StatusBadge ok={false} label={`${gitStatus.changedFiles} uncommitted`} />
              ) : (
                <StatusBadge ok label="clean" />
              )}
              {gitStatus.aheadCount > 0 && (
                <StatusBadge ok={false} label={`${gitStatus.aheadCount} unpushed`} />
              )}
            </div>
            ...
            <button
              className="ghost"
              style={{ fontSize: 12, alignSelf: 'flex-start', marginTop: 6 }}
          ...
          <div style={{ color: 'var(--text3)', fontSize: 13 }}>
            Not a git repo — navigate to a git project and run{' '}
            <code style={{ color: 'var(--accent)' }}>ccm project init</code> to enable
            checkpointing.
          </div>
        )}
        {status && (
          <div style={{ color: 'var(--accent)', fontSize: 12, marginTop: 8 }}>{status}</div>
        )}
      </div>
        ...
        {cfg.github.enabled && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, paddingLeft: 0 }}>
            ...
            {cfg.github.sessionBackup && (
              ...
              >
                <label style={{ color: 'var(--text2)', fontSize: 12 }}>
                  Backup repo URL (SSH or HTTPS)
                </label>
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button className="primary" style={{ fontSize: 12 }} onClick={saveBackupRepo}>
                    save
                  </button>
                </div>
                {testResult && (
                  <div
                    style={{
                      color: testResult.reachable ? 'var(--accent)' : 'var(--red)',
                      fontSize: 12,
                    }}
                  >
                    {testResult.reachable
                      ? '✓ Remote reachable'
                      : `✗ ${testResult.detail || 'Cannot reach remote'}`}
                  </div>
                )}
              </div>
            )}
      ...
      <div>
        ...
        {checkpoints.length === 0 ? (
          <div style={{ color: 'var(--text3)', fontSize: 13 }}>
            No checkpoints yet. They are created automatically on every account switch when Smart
            Resume is on.
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
