...
export default function Settings() {
  const [config, setConfig] = useState({
    autoSwitch: true, keepSessionLog: true, showNotifications: true,
  })
  const [saved, setSaved] = useState(false)

  useEffect(() => {
    window.ccm.config.load()
      .then(r => { if (r?.data) setConfig(cfg => ({ ...cfg, ...r.data })) })
      .catch(() => { /* use defaults */ })
  }, [])
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 560 }}>
      ...
      <section>
        <div style={{ color: 'var(--text2)', fontSize: 11, textTransform: 'uppercase',
          letterSpacing: 1, marginBottom: 8 }}>Behaviour</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {TOGGLES.map(t => (
            <div key={t.key} onClick={() => toggle(t.key)} style={{
      ...
      {/* Security */}
      <section style={{ background: 'var(--bg3)', border: '1px solid var(--border)',
        borderRadius: 8, padding: '16px 18px' }}>
        <div style={{ color: 'var(--text2)', fontSize: 11, textTransform: 'uppercase',
          letterSpacing: 1, marginBottom: 10 }}>Security</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6,
          color: 'var(--text2)', fontSize: 12 }}>
          <div>API keys are encrypted with <span style={{ color: 'var(--accent)' }}>AES-256-GCM</span> before being stored.</div>
          <div>The encryption key is machine-specific (~/.ccm/.key). Set a passphrase for portability:</div>
          <code style={{ background: 'var(--bg)', padding: '6px 10px', borderRadius: 4,
            color: 'var(--text)', marginTop: 4, display: 'block' }}>
          ...
          <div style={{ color: 'var(--text3)', marginTop: 4 }}>
            ...
            Use the same passphrase when importing accounts on a new machine.
          </div>
        </div>
      </section>
      ...
      <section style={{ background: 'var(--bg3)', border: '1px solid var(--border)',
        borderRadius: 8, padding: '16px 18px' }}>
        <div style={{ color: 'var(--text2)', fontSize: 11, textTransform: 'uppercase',
          letterSpacing: 1, marginBottom: 10 }}>Storage paths</div>
        {[
          ['Config dir',     '~/.ccm/'],
