...
export default function Sidebar({ page, setPage }) {
  ...
  useEffect(() => {
    window.ccm.accounts.getActive().then(setActive).catch(() => {})
  }, [page])
  ...
  return (
    <aside style={{
      ...
      <div style={{ padding: '8px 20px 20px', WebkitAppRegion: 'no-drag' }}>
        <div style={{ color: 'var(--accent)', fontWeight: 700, fontSize: 16, letterSpacing: 1 }}>ccm</div>
      ...
      <nav style={{ flex: 1, WebkitAppRegion: 'no-drag' }}>
        {NAV.map(n => (
          <button key={n.key} onClick={() => setPage(n.key)} style={{
            ...
            <span style={{ fontSize: 13, color: page === n.key ? 'var(--accent)' : 'var(--text3)' }}>{n.icon}</span>
            {n.label}
          </button>
        ))}
      </nav>

      <div style={{ padding: '12px 20px', borderTop: '1px solid var(--border)', WebkitAppRegion: 'no-drag' }}>
        {active ? (
          <div>
        ...
        <button className="ghost" style={{ marginTop: 10, width: '100%', fontSize: 12 }}
          onClick={() => setPage('add-account')}>+ add account</button>
      </div>
    </aside>
  )
}
