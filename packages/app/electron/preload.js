...
contextBridge.exposeInMainWorld('ccm', {
  ...
  projects: {
    ...
    bind: (projectRoot, accountName) =>
      ipcRenderer.invoke('projects:bind', { projectRoot, accountName }),
    init: (dir, accountName, name) =>
      ipcRenderer.invoke('projects:init', { dir, accountName, name }),
    checkGitignore: (projectDir, autoAdd) =>
      ipcRenderer.invoke('projects:check-gitignore', { projectDir, autoAdd }),
  ...
  sync: {
    ...
    checkpointDiff: (projectRoot, commitHash) =>
      ipcRenderer.invoke('sync:checkpoint-diff', { projectRoot, commitHash }),
    restoreCheckpoint: (projectRoot, commitHash) =>
      ipcRenderer.invoke('sync:restore-checkpoint', { projectRoot, commitHash }),
